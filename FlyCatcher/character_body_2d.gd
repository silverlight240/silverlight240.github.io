...
func _enter_tree() -> void:
	...
	$Area2D.area_entered.connect(_in_bush)
	add_to_group("player")
	randomize()
	...
	if thingy == true:
		if thingy15 == false:
			if thingy14 == false:
				...
				emit_signal("unselect", maxhp)
				queue_free()
func _in_bush(area: Area2D):
	if area.get_parent().is_in_group("bush"):
		if area.get_parent().picked == false:
			self.modulate = Color.GREEN_YELLOW
			berryxp += 1
			if berryxp >= 30:
				$Timer2.start()
				self.modulate = Color.NAVY_BLUE
				thingy13 = true
				if thingy == true:
					thingy = false
					emit_signal("unselect", maxhp)
...
func _on_timer_2_timeout() -> void:
	if exp >= 3:
		var new_object_instance = spawnguy2.instantiate()
		get_parent().get_parent().add_child(new_object_instance)
		new_object_instance.global_position = spawn_point.global_position
		queue_free()
	if berryxp >= 30:
		var new_object_instance = spawnguy1.instantiate()
		get_parent().get_parent().add_child(new_object_instance)
		new_object_instance.global_position = spawn_point.global_position
		queue_free()

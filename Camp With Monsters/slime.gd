...
func _on_area_2d_2_body_entered(body: Node2D) -> void:
	if body != self and body.is_in_group("slime") and not body.is_in_group("Bigslime") and not body.is_in_group("Hugeslime") and not body.is_in_group("queenslime"):
		body.queue_free()
		var spawn = load("res://Pod.tscn").instantiate()
